...
const LoginPage = ({ onNavigate }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      login(data);
      toast.success('Welcome back!', `Logged in as ${data.user?.first_name || phone}`);

      // FIX: role lives on data.user.role, not data.role
      const roleRoutes = {
        customer:      '/dashboard',
        agent:         '/agent',
        admin:         '/admin',
      ...
      onNavigate(roleRoutes[data.user?.role] || '/dashboard');
    } catch (err) {
  ...
  const fillDemo = (role) => {
    ...
    const d = demos[role];
    if (d) { setPhone(d.phone); setPassword(d.password); setError(''); }
  };
      ...
      <div className="login-left">
        <div style={{ position: 'relative', zIndex: 1, textAlign: 'center', color: 'white' }}>
          ...
          <p style={{ fontSize: 16, opacity: 0.7, lineHeight: 1.7, maxWidth: 400 }}>
            Send money, pay bills, save, and access loans — all from your phone.
            ...
            {[
              ...
              <div key={f.label} style={{ textAlign: 'center' }}>
                <div style={{
                  ...
                  <i className={`bi ${f.icon}`} style={{ color: 'var(--mpesa-green)' }} />
                </div>
        ...
        <div className="login-form-wrapper">
          ...
          <div className="login-logo">
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: 'var(--mpesa-green)', fontWeight: 700, letterSpacing: 1 }}>
                BANKING PORTAL
              </div>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} noValidate>
            ...
            <button
              ...
              style={{ marginTop: 8 }}
            >
              {!loading && <><i className="bi bi-box-arrow-in-right" /> Sign In</>}
            </button>
          </form>
